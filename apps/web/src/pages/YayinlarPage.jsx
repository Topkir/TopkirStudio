...
const releases = [{
  ep: 'YAYIN 0',
  date: '18 KAS 2025',
  title: 'Tabula fikri ve ilk prototip.',
  chips: ['🎮 TABULA', '💡 FİKİR']
}, {
  ep: 'YAYIN 1',
  date: '24 KAS 2025',
  ...
  ep: 'YAYIN 2',
  date: '10 OCA 2026',
  title: 'İlk 1.000 indirmeye ulaşıldı.',
  chips: ['🎮 TABULA', '📈 BÜYÜME']
}, {
  ep: 'YAYIN 3',
  date: '02 MAR 2026',
  title: 'App Store sürümü için hazırlıklar başladı.',
  chips: ['🎮 TABULA', '🍎 APP STORE']
}];
  ...
  return <div className="min-h-screen grain-bg">
      ...
      <div className="mx-auto max-w-4xl px-6 pt-10 pb-8 md:pt-16">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mt-14">
          <Eyebrow>// Sıfırdan Yayına</Eyebrow>
          <h1 className="font-display text-5xl md:text-7xl font-bold tracking-tight leading-[0.95] text-white">
            Topkir Studio<br />Yayınlar
          </h1>
          <p className="mt-6 text-lg text-white/50">Yayınlanan uygulamaların gelişim defteri.</p>

          <div className="mt-10 flex items-center gap-4">
            <div className="relative h-1.5 flex-1 rounded-full bg-white/10">
              <div className="absolute left-0 top-0 h-full w-[75%] rounded-full bg-emerald-400" />
              <div className="absolute left-[75%] top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-emerald-400 ring-4 ring-emerald-400/20" />
            </div>
            <span className="font-mono-ui text-xs tracking-[0.15em] text-white/50">4 YAYIN</span>
          </div>
        </motion.div>

        <motion.div variants={container} initial="hidden" animate="show" className="mt-12 space-y-4">
          {releases.map(u => <motion.div key={u.ep} variants={item} className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-6 transition-colors hover:border-white/15">
